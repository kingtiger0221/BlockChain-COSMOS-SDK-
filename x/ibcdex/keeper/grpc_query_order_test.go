...
func TestOrderQuerySingle(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{
			desc:     "First",
			request:  &types.QueryGetOrderRequest{Index: msgs[0].Index},
			response: &types.QueryGetOrderResponse{Order: &msgs[0]},
		},
		{
			desc:     "Second",
			request:  &types.QueryGetOrderRequest{Index: msgs[1].Index},
			response: &types.QueryGetOrderResponse{Order: &msgs[1]},
		},
		{
			desc:    "KeyNotFound",
			request: &types.QueryGetOrderRequest{Index: "missing"},
